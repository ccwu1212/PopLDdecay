...
if ( $method eq "HW" || $method eq "MedianBin" || $method eq "PercentileBin" || $method eq "Percentile" ) # HW =="Hill and Weir"
{
	...
	{
		print "The input result only with measure [r2], can't find the D', To calculate the D' result , you should run the core program [PopLDdecay] with parameter [ -OutType 1]\n";
