...
if ($method  eq "MeanBin" || $method  eq "Mean" ||  $method  eq "mean" )  
{
	...
	while(<LIST>)
	{
		...
		{
			print "The $PopID input result only with measure [r2], can't find the D', To calculate the D' result , you should run the core program [PopLDdecay] with parameter [ -OutPairLD 2]\n";
...
my $R="/hwfswh2/BC_PUB/Software/03.Soft_ALL/R-3.5.1/bin/Rscript";
if  ( !(-e $R) )
{
